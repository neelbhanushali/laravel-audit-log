...
trait Auditable
{
    ...
    protected static function bootAuditable()
    {
        ...
        static::updated(function ($record) {
            ...
            // restore fires `updating` and `updated` events internally
            if(count($audit_trail->difference) == 0) {
                return;
            }
        ...
        if (in_array(SoftDeletes::class, $reflection->getTraitNames())) {
            static::restored(function ($record) {
                ...
                $audit_trail->before_transaction = [];
                $audit_trail->after_transaction = self::removeTimestamps($record, $record->attributes);
                $audit_trail->difference = $before_transaction->after_transaction;
    
                $audit_trail->activity = 'restore';
                $audit_trail->user_id = request()->user() ? request()->user()->id : null;
